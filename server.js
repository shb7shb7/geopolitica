...
const EVENT_POOL = [
  ...
  // Underdog events — weight 2 = 2x plus fréquents, seuils recalibrés
...
const PERIODS = [
  { number:1, name:"L'Ère de Croissance",       subtitle:"Janvier — Juin · An 1",    desc:"Les marchés mondiaux s'ouvrent. Investissez et posez les bases de votre empire." },
  { number:2, name:"Premières Tensions",         subtitle:"Juillet — Déc · An 1",     desc:"Les intérêts divergent. Les premières frictions commerciales apparaissent." },
  { number:3, name:"Crise Mondiale",             subtitle:"Janvier — Juin · An 2",    desc:"Une catastrophe majeure frappe la planète. Personne n'est épargné." },
  { number:4, name:"Course aux Armements",       subtitle:"Juillet — Déc · An 2",     desc:"La guerre se profile. Les nations investissent massivement dans leur défense." },
  { number:5, name:"Ultimatums",                 subtitle:"Janvier — Juin · An 3",    desc:"Les diplomates ont échoué. Sanctions et alliances de dernière minute." },
  { number:6, name:"Le Monde Retient son Souffle", subtitle:"Juillet — Déc · An 3",  desc:"Dernière période de paix. Chaque décision compte. Les canons parleront bientôt." },
];
...
let gameState = {
  ...
  eventMod:{ armyCostMult:1, cheapAttack:false, peace:false, blackMarket:false, blackMarketThreshold:4500 },
...
function applyEvent(ev) {
  ...
  gameState.eventMod = { armyCostMult:1, cheapAttack:false, peace:false, blackMarket:false, blackMarketThreshold:4500 };
  if (ev.priceChanges) {
...
function applyPeriodTransition() {
  Object.values(gameState.countries).forEach(c => {
    ...
    if (c.food >= foodNeeded) {
      ...
      addTeamNews(c.team, `⚠️ Famine ! −${powerLoss} pts de puissance`, 'bad');
    }
...
io.on('connection', (socket) => {
  ...
  socket.on('team:declareAttack', ({teamName, targetId}) => {
    ...
    if (result.attackerWins) {
      ...
      addTeamNews(att.team,`🏆 VICTOIRE contre ${def.flag} ${def.name} ! Toutes leurs ressources pillées. Pertes: −${armyLossAtt} armée`,'good');
      addTeamNews(def.team,`💀 DÉFAITE contre ${att.flag} ${att.name} — toutes vos ressources pillées. Pertes: −${armyLossDef} armée`,'bad');
      ...
      addTeamNews(def.team,`🛡️ Vous avez repoussé ${att.flag} ${att.name} ! Pertes: −${armyLossDef} armée`,'good');
  ...
  socket.on('mj:reset', () => {
    if (timerInterval) clearInterval(timerInterval); timerInterval=null;
    gameState={ phase:'setup', currentPeriod:0, currentEvent:null, countries:{}, takenCountries:{}, teams:{}, prices:{oil:80,food:40}, eventMod:{armyCostMult:1,cheapAttack:false,peace:false,blackMarket:false,blackMarketThreshold:4500}, coalition:{proposed:false,moroccoAccepted:false,guineaAccepted:false,active:false}, log:[], timerSeconds:600, timerRunning:false };
    broadcast();
